...
// СМЯГЧЕННАЯ ПРОВЕРКА: Если полифиллов нет — скрипт больше не падает, а делает безопасную заглушку
let inlinedLegacyRuntimeScript = "";
if (legacyRuntimeFile) {
  const legacyRuntimeSource = readClientFile(legacyRuntimeFile);
  inlinedLegacyRuntimeScript = `    <script id="vite-systemjs-loader">\n${escapeInlineScript(
    legacyRuntimeSource,
  )}\n    </script>`;
} else {
  console.log("[capacitor-html] Warning: No legacy polyfill runtime found. Injecting empty loader safety asset.");
  inlinedLegacyRuntimeScript = `    <script id="vite-systemjs-loader">// No external runtime needed</script>`;
}
...
// СМЯГЧЕННАЯ ПРОВЕРКА: Если легаси-чанка нет, создаем пустую строку вместо падения
let legacyEntryTag = "";
if (legacyEntryFile) {
  legacyEntryTag = `    <script nomodule crossorigin id="vite-legacy-entry" data-src="${htmlAssetPath(
    legacyEntryFile,
